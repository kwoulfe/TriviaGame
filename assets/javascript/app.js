...
[{
	...
	question: "What was the name of the male support garment that Kramer created for Frank Costanza?",
	choices: ["The Manzier", "Spankys", "The Bro", "The Cup"],
	answer: 0
},{	
	question: "Who supplied the voice for Yankees boss George Steinbrenner?",
	choices: ["George Steinbrenner", "Larry David", "Jerry Seinfeld", "Jason Alexander"],
	answer: 1
},{	
	question: "Which New York Met was 'second spitter' in the 'JFK' parody sequence featuring Keith Hernandez?",
	...
	question: "What embarrassment did George endure when he visited The Hamptons?",
	choices: ["He double dipped his chips at a fancy party", "He didn't bring enough money to dinner", "His car had a smell that he couldn't get rid of", "Shrinkage"],
	answer: 3
},{	
	...
	choices: ["fight and vow to never speak again.", "end up in jail.", "move to Singapore.", "all die."],
	answer: 1
}];
    ...
    displayCurrentQuestion();
		...
		$(this).find(".nextButton").on("click", function() {
			if (!quizOver) {
				...
				if (value == undefined) {
					$(document).find(".result").text("Pick an answer dummy!");
...
function displayCurrentQuestion() {
	...
	$(quizChoices).find("li").remove();
	var choice;
		for (i = 0; i < answerList; i++) {
			choice = questions[currentQuestion].choices[i];
			$('<li><input type="radio" value=' + i + ' name="dynradio" /> ' + choice + '</li>').appendTo(quizChoices);
		}
}
...
function count() {
	...
	if (time < 0) {
		...
		$(document).find(".nextButton").text("Try Again?");
		$(document).find(".quizChoices").hide();
		$(document).find(".quizQuestion").hide();
	}
}
...
function showScore() {
	$(document).find(".container > .score").text(correctAnswers + " out of " + questions.length);
	$(document).find(".container > .score").show();
}
